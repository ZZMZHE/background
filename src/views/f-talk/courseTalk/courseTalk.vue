...
import { commentCourseList, commentCourseUpdate, commentCourseDel, commentCourseUpdateIsHide, commentCourseUpdateStatus } from 'api/index.js'
...
export default {
    ...
    methods: {
        ...
        flagChange(val) {
            ...
            commentCourseUpdateIsHide(``, { id: val.id, status }).then(res => {
                if (res.status === 200) {
                    this.getData()
                }
            })
        ...
        aduit(val) {
            ...
            this.$confirm(`此操作将审核该评论, 是否${txt}?`, '提示', {
                ...
                commentCourseUpdateStatus(``, { id: val.id, status: val.status }).then(res => {
                    if (res.status === 200) {
        ...
        del(val) {
            this.$confirm('此操作将删除该评论, 是否继续?', '提示', {
                ...
                commentCourseDel(``, { id: val.id }).then(res => {
                    if (res) {
        ...
        // 去详情
        goDetails(val) {
            this.$router.push({ name: 'courseCommentDetails', query: val })
        }
    },
    created() {
        this.getData()
