            ...
            <div class="header">
                <el-form :inline="true" :model="formInline" ref="formInline" class="demo-form-inline" size="small">
                    <el-form-item label="资讯ID：" prop="id">
                        <el-input v-model="formInline.id" placeholder="请输入" class="phoneId"></el-input>
                    </el-form-item>
                    <!-- <el-form-item label="订单编号：" prop="orderSn">
                        <el-input v-model="formInline.orderSn" placeholder="请输入" class="phoneId"></el-input>
                    </el-form-item> -->
                    <el-form-item label="昵称：" prop="name">
                        <el-input v-model="formInline.name" placeholder="请输入" class="phoneId"></el-input>
                    </el-form-item>
                    <el-form-item label="发表时间" prop="dateValue">
                        <el-date-picker
...
import { CommentNewsInformationList, CommentNewsInformationUpdate, CommentNewsInformationDel, CommentNewsInformationUpdateIsHide, CommentNewsInformationUpdateStatus } from 'api/index.js'
...
export default {
    ...
    methods: {
        ...
        flagChange(val) {
            ...
            CommentNewsInformationUpdateIsHide(``, { id: val.id, status }).then(res => {
                if (res.status === 200) {
                    this.getData()
                }
            })
        ...
        aduit(val) {
            ...
            this.$confirm(`此操作将审核该评论, 是否${txt}?`, '提示', {
                ...
                CommentNewsInformationUpdateStatus(``, { id: val.id, status: val.status }).then(res => {
                    if (res.status === 200) {
        ...
        del(val) {
            this.$confirm('此操作将删除该评论, 是否继续?', '提示', {
                ...
                CommentNewsInformationDel(``, { id: val.id }).then(res => {
                    if (res) {
