...
<script>
import { cmsBannerEduCategoryList, cmsBannerEduCategoryDel } from 'api/index.js'
export default {
    ...
    methods: {
        // 获取数据
        getData(val) {
            cmsBannerEduCategoryList().then(res => {
                if (res.status === 200) {
        ...
        goDetails(val) {
            ...
            // }
            this.$router.push({ name: 'educationBannerMidDetails', query: val })
        }
    },
    created() {
        this.getData()
