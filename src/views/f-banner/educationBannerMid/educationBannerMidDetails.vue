...
<script>
import { cmsBannerEduCategoryAdd, cmsBannerEduCategoryInfo } from 'api/index.js'
export default {
    ...
    computed: {
        uploadUrl() {
            return this.$store.state.uploadUrl + '63'
        }
    },
    watch: {},
    methods: {
        getData() {
            cmsBannerEduCategoryInfo(``, {
                id: this.id
            }).then(res => {
                if (res.status === 200) {
        ...
        // 返回上一页
        goBack() {
            this.$router.push({ name: 'educationBannerMid' })
        }
    },
    created() {
