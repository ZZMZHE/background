<template>
    <div class="eductionBottomBanner">
        <div class="conn">
            <div class="header">
...
<script>
import { cmsBannerEduCentreList, cmsBannerEduCentreDel } from 'api/index.js'
export default {
    name: 'eductionBottomBanner',
    components: {},
    data() {
        return {
            tableData: []
    ...
    methods: {
        // 获取数据
        getData(val) {
            cmsBannerEduCentreList().then(res => {
                if (res.status === 200) {
        ...
        goDetails(val) {
            if (val.type === 1 && this.tableData.length >= 1) {
                return this.$message.error('在线教育下方banner最多一张！')
            }
