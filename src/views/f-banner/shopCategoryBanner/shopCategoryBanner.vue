...
<script>
import { cmsBannerMallCategoryList, cmsBannerMallCategoryDel } from 'api/index.js'
export default {
    ...
    methods: {
        // 获取数据
        getData(val) {
            cmsBannerMallCategoryList().then(res => {
                if (res.status === 200) {
        ...
        goDetails(val) {
            ...
            //     return this.$message.error('在线教育banner最多三张！')
            // }
            this.$router.push({ name: 'shopCategoryBannerDetails', query: val })
        }
    },
    created() {
        this.getData()
