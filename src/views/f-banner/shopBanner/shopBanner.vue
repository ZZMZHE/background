...
<script>
import { cmsBannerMallHeadList, cmsBannerMallHeadDel } from 'api/index.js'
export default {
    name: 'shopBanner',
    components: {},
    data() {
    ...
    methods: {
        // 获取数据
        getData(val) {
            cmsBannerMallHeadList().then(res => {
                if (res.status === 200) {
        ...
        goDetails(val) {
            if (val.type === 1 && this.tableData.length >= 5) {
                return this.$message.error('商城banner最多五张！')
            }
            this.$router.push({ name: 'shopBannerDetails', query: val })
        }
    },
    created() {
        this.getData()
