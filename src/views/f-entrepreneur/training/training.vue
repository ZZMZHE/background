...
<script>
import {
    getCmsEnterpriseServicesDetailVo,
    updateCmsEnterpriseServices
} from "api/index.js";
...
export default {
    ...
    methods: {
        ...
        All() {
            let then = this;
            getCmsEnterpriseServicesDetailVo("", {}).then(res => {
                if (res.status == 200) {
                    then.courseData.coverUrl = res.data.imgSrc;
                    then.courseData.id=res.data.id
                    then.courseData.detail=res.data.internalTraining
                    if (res.data.internalTraining) {
                        setTimeout(() => {
                            this.$refs.text.getData(
                                res.data.internalTraining
                            );
                        }, 300)
                    }
                }
            ...
            this.$confirm("此操作将修改该文件, 是否继续?", "提示", {
                ...
                .then(() => {
                    ...
                    updateCmsEnterpriseServices("", {
                        ...
                        internalTraining: courseData.detail,
